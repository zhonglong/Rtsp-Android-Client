...
public class H264Stream extends VideoStream {
        ...
        public void run() {
            ...
            while (!Thread.interrupted() && !isStop) {
                try {
                    ...
                    if (framType == 5 || framType == 6) startKeyFrame = true;
                    if (startKeyFrame || framType == 7 || framType == 8) {
                        inputBufferIndex = mMeidaCodec.dequeueInputBuffer(2000);
                        if (inputBufferIndex >= 0) {
                            ByteBuffer inputBuffer;
                            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                                inputBuffer = mMeidaCodec.getInputBuffers()[inputBufferIndex];
                            } else {
                                inputBuffer = mMeidaCodec.getInputBuffer(inputBufferIndex);
                            }
                            inputBuffer.clear();
    ...
    private void decodeSPS() {
        ...
        header_pps = new byte[pps.length + 4];
        header_sps = new byte[sps.length + 4];
        header_pps[0] = header_sps[0] = 0x00;
        header_pps[1] = header_sps[1] = 0x00;
        header_pps[2] = header_sps[2] = 0x00;
        header_pps[3] = header_sps[3] = 0x01;
        System.arraycopy(sps, 0, header_sps, 4, sps.length);
        System.arraycopy(pps, 0, header_pps, 4, pps.length);
